...
A ML model and app for detecting human speech in outdoor ecological acoustic datasets.
...
Additionally, the app provides multiple output types with all the detections. The plain CSV outputs are easily imported to data analysis scripts in programs like R, and additional outputs are provided for direct import into other spectrogram analysis programs (Raven, Kaleidoscope, and Audacity) aiding studies of anthropogenic effects on animal behaviour.
...
The user can return to the start-up screen from the project workspace by selecting `File > Close Project` or `ctrl + W`
...
*Navigate Detections*
- The table can be navigated with arrow keys as expected. Beginning to type will add values to the cell with a black shadow where you've navigated to.
...
- Editing the Start Time and End Time selection box above `Play`, then selecting `Add` will create a new detection in the table at the specified start and end times.
- Highlighting one or more rows in the table and selecting `Delete` removes these rows from the visible table and associated `project_review.csv` file. All original detections from the automated voice detection stage are retained in the `project_detections.csv`.
...
- *ID*: Unique identifier for this detection. These IDs are retained when the `project_review.csv` is created.
...
#### project_review.csv
After opening and closing the review screen for the first time after the voice detector is run, in `Softspoken/projects/Softspoken Outputs/[project-name]` this file will be created. Every time the review screen is open and closed this file, the Audacity, Kaleidoscope, and Raven outputs are updated.
...
The file contains the following headers:
- *ID*: original IDs will be retained from the `project_detections.csv` unless deleted during review process. When rows are added in review detections screen, they will recieve a new unique identifier number.
...
*To view in Audacity:*
- In a new Audacity window, use `File > Open...` to select a `.wav` file that was reviewed by softspoken, or drag and drop the `.wav` file.
